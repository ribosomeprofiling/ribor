...
In the last two sections, we describe some [advanced features](#advancedfeatures)
...
Here, we outline a relatively simple analysis. The goal of this is to show some of the capabilities of the package and provide a brief example on what can be done in further analyses with a few base R functions. 

A potential use case of the RNA-Seq data (example seen in the `RNA-Seq`section in `Optional Data`) is to measure translational efficiency or plot it alongside the ribosome profiling region counts. By having the region-specific read counts of both RNA-Seq and ribosome profiling data, we can first filter out any lowly expressed transcripts, and visualize the relationship between the RNA-Seq and ribosome profiling region counts.
...
From the above plots, we can roughly estimate the threshold to filter out the lowly expressed transcripts, and for simplicity, we roughly estimate a value of 2 (which is 1 on the log2 scale) for both the RNA-Seq and region counts. After this, we make a scatterplot of the region counts over the RNA-Seq counts.

``` {r filter_and_plot}
# filter out transcripts with count value less than 2 for either region counts or rnaseq
...
plot(log2(rnaseq_CDS$count), log2(rc_CDS$count), xlab = "Log 2 CDS RNA-Seq Count", ylab = "Log 2 of CDS Region Counts", main = "Scatter Plot of Ribosome Occupancy and RNA Abundance")
```

Now, using the filtered data, we can look at the transcripts with a high translational efficiency. To do so, we divide the region counts by the RNA-Seq counts, and plot the density distribution of these ratios. From there, we visually choose the threshold again as a simple way to find the transcripts with a relatively high translational efficiency.

``` {r translational_efficiency}
# get the translational efficiency of CDS 
te_CDS    <- rc_CDS$count/rnaseq_CDS$count
plot(density(log2(te_CDS)), xlab = "Log 2 of Translational Efficiency", main = "Translational Efficiency Density")
```

From the above density distribution, we see the small elbow around -2 on the log2 scale, and we choose to use that as a rough estimate of the threshold value. From this threshold value, we find 229 transcripts that exhibit relatively high translational efficiency. 
