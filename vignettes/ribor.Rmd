...
output:
  ...
  word_document:
    ...
    toc_depth: '4'
vignette: |
  %\VignetteIndexEntry{A Walkthrough of RiboR}  %\VignetteEngine{knitr::rmarkdown}  %\VignetteEncoding{UTF-8}
---
...
Actively translating messenger RNAs are engaged with ribosomes and protein synthesis rates can be approximated by the number of ribosomes that are translating a given mRNA. 
...
Ribosome profiling data analyses involve several quantifications for each transcript. Specifically, the lengths of the RPFs provide valuable biological information (see, for example, [Lareau et al.](https://elifesciences.org/articles/01257) and [Wu et al.](https://www.sciencedirect.com/science/article/pii/S1097276518310633?via%3Dihub)). To facilitate ribosome profiling data analyses as a function of RPF length in a highly efficient manner, we implemented a new data format, called [ribo](https://ribogadgets.readthedocs.io/en/latest/ribo_file_format.html). 
Files in ribo format are called __.ribo__ files.


RiboR package is an R interface for .ribo files. The package offers a suite of reading functions for data-sets in  .ribo files, and provides plotting functions that are most often employed in ribosome profiling analyses.


Using RiboR, one can import .ribo files to R environment, read ribosome profiling data into [data tables](https://cran.r-project.org/web/packages/data.table/index.html) and generate essential plots in a few lines of R code.

This document is organized as follows. In the rest of this section, we give an overview of the 
[Ribo File format](#ribofileformat) and introduce [transcript regions](#transcriptregions).
Then we provide instructions and requirements for the [installation](installation) of RiboR.
To start analyzing data, we show how to import a .ribo file to R environment in the section 
[Getting Started](#gettingstarted).
Next we explain how to perform essential ribosome profiling data analyses:

   * [Length Distribution](#lengthdistribution)
   * [Metagene Analysis](#metageneanalysis)
   * [Region Counts](#regioncounts)
   
We continue with going over [advanced features](#advancedfeatures). 
The first advanced feature is renaming transcripts using [aliases](#alias). 
The second is accessing .ribo [file attributes](#fileattributes).
As the third feature, we show how to obtain [annotation](#annotation) that defines transcript regions.  
...
* [Coverage](#coverage): A .ribo file can be created to keep the nucleotide level transcriptome coverage.
...
For a more detailed explanation of the ribo format, we refer to 
[readthedocs page of ribo](https://ribogadgets.readthedocs.io/en/latest/ribo_file_format.html).
Briefly, a .ribo file is built on an [HDF5](https://www.hdfgroup.org/solutions/hdf5/) architecture and has a predefined internal structure (Figure 1). 
...
### Requirements
* **R:** RiboR requires **version 3.6 or later**. Previous R versions are known to cause installation or run time problems.
...
[online documentation of the ribo format](https://ribogadgets.readthedocs.io/en/latest/ribo_file_format.html). 
...
We use the function `plot_length_distribution` to generate the distribution of the reads mapping to a specific region. This function has also a boolean argument called `fraction`. When fraction is `FALSE`, the y-axis has the total number of reads mapping to the specified region. When fraction is `TRUE`, the y-axis has the number of reads mapping to the specified region divided by the total number of transcriptome mapping reads.  

The following code takes the ribo object as input and plots the length distribution of RPFs that mapped to coding regions in the range of 28 to 32 nucleotides.  
...
We can get the numerical values producing the above plot using the function `get_length_distribution` as follows.
...
It is also possible to generate the above plot from the data.table returned by `get_region_counts`.
This can be useful if the user wants to further process the data prior to plotting.

```{r plot_length_distribution_from_datatable}
...
## Metagene Analysis {#metageneanalysis}
A typical quality control step in ribosome profiling analyses is the inspection of the pileup of sequencing reads with respect to the start and stop site of annotated coding regions. Given that ribosomes are expected to be translating coding regions, these plots are informative about the enrichment at the boundaries of coding regions and also provide information regarding the periodicity of aligned sequencing reads. This type of plot is called a metagene plot as the reads are aggregated around translation start and stop sites across all transcripts.

The region that is analyzed for these plots is defined by the parameter “metagene radius”. This value is simply the number of nucleotides surrounding the start/stop site that is considered.  For each position, relative to the start/stop site, the coverage values are aggregated across transcripts. The cumulative read coverage is plotted on the y-axis as a function of  the position relative to the start/stop site on the x-axis. 

We can identify the metagene radius used to generate our .ribo file using the `get_info` function.
``` {r learn_metagene_radius}
get_info(original.ribo)$attributes$metagene_radius
```

Using `plot_metagene`, we can plot the ribosome occupancy around the start or stop sites as described above. Specifically, the following code will produce the metagene plot at the start site for the experiment GSM1606107. The values on the y-axis are the raw read counts.
...
We can also change the default title to a different one, for example, by setting `title = "Stop Site Coverage"`. 
...
While [tidy data](https://tidyr.tidyverse.org/) is a commonly used standard in R, especially in
[tidyverse](https://tidyr.tidyverse.org/), in this particular case, it reports each nucleotide position in a separate row. 
On the other hand, the non-tidy option provides a more compact representation of the metagene data.
...
Lastly, to preserve information about the counts at each individual read length and at each individual transcript,
use `transcript = FALSE` and `length = FALSE`. 
Because there are often thousands of transcripts, this is usually the most computationally intense option in the retrieval of the metagene data. 
The raw data set is read and no summation operation is performed. 
For a typical ribosome profiling experiment, run times might be slower for this option, 
...
#print first ten columns and first two rows
print(meta.start[1:2, 1:10])
```

We can generate metagene plots using the data tables returned by `get_metagene`.
...
It is well-known that ribosome pauses, or moves slower, around start and stop sites. As a result, we observe peaks around start and stop sites in metagene plots. This behavior of ribosome makes it harder to perform certain
analyses such as coverage, translation efficiency, periodicity and uniformity analysis with accuracy. 
...
This way, we keep regions around start and stop sites seperate when doing such analyses.
...
To get the data only summed across the read lengths, set `length = TRUE` and `transcript = FALSE`.
...
By default, the data tables are in tidy form. If non-tidy data set is desired, then set `tidy = FALSE`.
...
Similar to the case of metagene and length distribution, bar plot above can be generated directly from data tables.
...
If we want a returned list of previously printed attributes, then the function `get_info` will return all of the attributes found in the root of the .ribo file as well as information on each of the experiments. For a detailed explanation, see [the readthedocs page of RiboGadgets](https://ribogadgets.readthedocs.io).
...
Length distribution, metagene coverage and region counts are essential to ribosome profiling data analysis and
these data exist in every .ribo file. However, for certain types of analysis, additional data might be required.
...
To obtain data for fundamental ribosome profiling analysis such as metagene and region counts, we map the reads to the transcriptome and take the first nucleotide on the 5’ end of each mapped read. The distribution of all reads coming from an experiment is called coverage data. In other words, coverage data gives us the total number of reads mapping to each nucleotide position in the transcriptome.
